...
namespace _05.FootballTeamGenerator.Models
{
    public class Stats
    {
        ...
        public double AverageStat => (this.Endurance + this.Sprint + this.Shooting + this.Dribble + this.Passing) / STATS_COUNT;

        private void ValidateStat(string name, int value)
        {
            if (value < MIN_STAT || value > MAX_STAT)
            {
                throw new ArgumentException(String.Format(GlobalConstants.INVALID_STAT_EXC_MSG, name, MIN_STAT, MAX_STAT));
            }
        }
    }
}
